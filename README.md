...
Original repository: [https://github.com/Age-Of-Ages/JumpchainCharacterBuilder](https://github.com/Age-Of-Ages/JumpchainCharacterBuilder)
...
Randomizer state (lists, groups, entries) is migrated through [`003_randomizer_lists.sql`](apps/desktop/src/db/migrations/003_randomizer_lists.sql) so contributors can trace how data persists between releases.

#### History export & presets
- Use **Export History** in the randomizer to download a `.json` snapshot of recent draws. The payload structure is generated by [`createHistoryExportPayload`](apps/desktop/src/modules/randomizer/weightedPicker.ts), which normalizes run metadata for sharing or debugging.
- Saved presets (draw scope, counts, filters, seeds) live in `window.localStorage` under the `jumpchain.randomizer.presets.v1` key. Loading the app rehydrates presets from this namespace; clearing it wipes any stored configurations.
